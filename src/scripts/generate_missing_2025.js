...
async function main() {
  ...
  // Find all 2025 Paper 3 questions
  const paper3Questions = questions.filter(
    (q) => q.paper === "paper3" && q.year === 2025
  );
  ...
  for (const q of paper3Questions) {
    ...
    const hasExpl =
      ...
      explanations.paper3[q.section]["2025"] &&
      explanations.paper3[q.section]["2025"][numStr];

    if (!hasExpl || explanations.paper3[q.section]["2025"][numStr].trim().length < 100) {
      missing.push(q);
    }
  }

  console.log(`Found ${missing.length} missing questions for Paper 3, 2025.`);
  if (missing.length === 0) {
    console.log("No missing questions to generate! All 2025 questions are present.");
    return;
  }
  ...
  for (const q of missing) {
    console.log(`\nGenerating explanation for Paper 3, 2025, Section: ${q.section}, Q: ${q.number}...`);
    ...
    const systemInstruction = 
      ...
      "Format rules:\n" +
      "1. Start directly with '<h3><b>UPSC ISS Statistics Paper III (2025) — Model Solution</b></h3><br>'.\n" +
...
`;
    ...
    while (!success && attempts < 5) {
      ...
      try {
        ...
        if (!explanations.paper3[q.section]["2025"]) explanations.paper3[q.section]["2025"] = {};
        
        explanations.paper3[q.section]["2025"][String(q.number)] = text.trim();
        success = true;
  ...
  console.log("\nAll missing explanations for Paper 3 2025 have been processed and saved!");
}
