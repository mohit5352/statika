...
async function main() {
  ...
  // Find all 2016 Paper 3 questions
  const paper3Questions = questions.filter(
    (q) => q.paper === "paper3" && q.year === 2016
  );
  ...
  for (const q of paper3Questions) {
    ...
    const hasExpl =
      ...
      explanations.paper3[q.section]["2016"] &&
      explanations.paper3[q.section]["2016"][numStr];

    if (!hasExpl || explanations.paper3[q.section]["2016"][numStr].trim().length < 100) {
      missing.push(q);
    }
  }

  console.log(`Found ${missing.length} missing questions for Paper 3, 2016.`);
  if (missing.length === 0) {
    console.log("No missing questions to generate! All 2016 questions are present.");
    return;
  }
  ...
  for (const q of missing) {
    console.log(`\nGenerating explanation for Paper 3, 2016, Section: ${q.section}, Q: ${q.number}...`);
    ...
    const systemInstruction = 
      ...
      "Consult and base your solutions on standard authoritative textbooks:\n" +
      "- Sampling Theory: Cochran (Sampling Techniques), Sukhatme & Sukhatme, Singh & Chaudhary.\n" +
      "- Econometrics: Johnston & DiNardo, William Greene (Econometric Analysis), Damodar Gujarati.\n" +
      "- Applied Statistics / Index Numbers / Demand Analysis: Henderson & Quandt (Microeconomic Theory), Koutsoyiannis, Croxton & Cowden.\n" +
      "- Time Series Analysis: Box, Jenkins & Reinsel, Brockwell & Davis, Chatfield.\n\n" +
      "Format rules:\n" +
      "1. Start directly with '<h3><b>UPSC ISS Statistics Paper III (2016) — Model Solution</b></h3><br>'.\n" +
...
`;
    ...
    while (!success && attempts < 12) {
      ...
      try {
        ...
        if (!explanations.paper3[q.section]["2016"]) explanations.paper3[q.section]["2016"] = {};
        
        explanations.paper3[q.section]["2016"][String(q.number)] = text.trim();
        success = true;
        ...
        if (attempts >= 12) {
          console.error(`Skipping Q: ${q.number} after 12 failed attempts.`);
        } else {
  ...
  console.log("\nAll missing explanations for Paper 3 2016 have been processed and saved!");
}
