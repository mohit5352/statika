...
async function main() {
  ...
  // Find all 2017 Paper 3 questions
  const paper3Questions = questions.filter(
    (q) => q.paper === "paper3" && q.year === 2017
  );
  ...
  for (const q of paper3Questions) {
    ...
    const hasExpl =
      ...
      explanations.paper3[q.section]["2017"] &&
      explanations.paper3[q.section]["2017"][numStr];

    if (!hasExpl || explanations.paper3[q.section]["2017"][numStr].trim().length < 100) {
      missing.push(q);
    }
  }

  console.log(`Found ${missing.length} missing questions for Paper 3, 2017.`);
  if (missing.length === 0) {
    console.log("No missing questions to generate! All 2017 questions are present.");
    return;
  }
  ...
  for (const q of missing) {
    console.log(`\nGenerating explanation for Paper 3, 2017, Section: ${q.section}, Q: ${q.number}...`);
    ...
    const systemInstruction = 
      ...
      "Format rules:\n" +
      "1. Start directly with '<h3><b>UPSC ISS Statistics Paper III (2017) — Model Solution</b></h3><br>'.\n" +
...
`;
    ...
    while (!success && attempts < 12) {
      ...
      try {
        ...
        if (!explanations.paper3[q.section]["2017"]) explanations.paper3[q.section]["2017"] = {};
        
        explanations.paper3[q.section]["2017"][String(q.number)] = text.trim();
        success = true;
  ...
  console.log("\nAll missing explanations for Paper 3 2017 have been processed and saved!");
}
